...
Vector.prototype.distance_from_line = function (line) {
  const from_start = this.difference(line.start);
  const from_end = this.difference(line.end);
  const start_to_end = line.end.difference(line.start);

  const distance_start_to_end = start_to_end.magnitude();
  const distance_from_start = from_start.magnitude();
  const distance_from_end = from_end.magnitude();
  ...
  const angle_from_this = from_start.angle();
  const angle_to_end = start_to_end.angle();
  const angle_from_start = angle_from_this - angle_to_end;
