...
Ball.prototype.end_update = function () {
...
Ball.prototype.resolve_cushion_impact = function (impact) {
  const speed = impact.dot_product(this.velocity);
  if (speed > 0) {
...
Ball.prototype.do_cushion = function (table) {
  ...
  for (let i in table.cushions) {
    const cushion = table.cushions[i];
    const impact = cushion.ball_impact_vector(this);
    if (impact) {
...
Ball.prototype.do_friction = function () {
  const velocity = this.ground_velocity();

  const swerve = this.spin.normal().scale(this.side);
...
Ball.prototype.do_collision = function (other) {
  ...
  let separation = this.position.difference(other.position);
  ...
  let correction = -1;
  for (let i = 0; i < 20; i++) {
    const corrected = separation.clone().add_scaled(this.velocity, correction);
  ...
  const collision_unit = separation.unit();
  const collision_speed = collision_unit.dot_product(this.velocity);
...
Ball.prototype.is_potted = function (pockets, position) {
  ...
  for (let p in pockets) {
    const sep = position.distance_from(pockets[p].position);
...
Ball.prototype.find_overlapping_ball = function (balls) {
  for (let i in balls) {
    const other = balls[i];
    if (this !== other) {
      const sep = this.position.distance_from(other.position);
...
Ball.prototype.is_legal_ball_in_hand_position = function (table, position) {
  ...
  for (let i in table.cushions) {
    const impact = table.cushions[i].ball_impact_vector(this, position);
    if (impact) return false;
  }

  return !this.is_potted(table.pockets, position);


}

Ball.prototype.blocks_path = function (line) {
  const distance = this.position.distance_from_line(line);
