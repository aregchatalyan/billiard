...
ShotCandidate.direct_shot = function (
  ...
  const path = ShotCandidatePath.direct(cueball, object_ball, pocket);
  return new ShotCandidate(table, cueball, null, path);
}

ShotCandidate.pocketless_shot = function (
  table, cueball, aimpoint, object_ball) {
  const target = { 'aimpoint': aimpoint };
  const path = ShotCandidatePath.direct(cueball, object_ball, target);
  return new ShotCandidate(table, cueball, null, path);
}

ShotCandidate.random_shot = function (table, cueball, random_aimpoint) {
  const target = { 'aimpoint': random_aimpoint };
...
ShotCandidate.cushion_shot = function (
  ...
  const path = ShotCandidatePath.one_cushion(
...
ShotCandidate.canon_shot = function (
  ...
  const path = ShotCandidatePath.direct(object_ball, pot_ball, target);
  if (!path) return null;
  return ShotCandidate.cushion_shot(
...
ShotCandidate.prototype.draw = function (ctx) {
  ...
  const end = this.cueball_destination;
  if (!end) return;
  ctx.beginPath();
...
ShotCandidate.prototype.get_strike_point = function () {
  const center = this.cueball.position;
  const max = this.cueball.radius;
  const direction = center.difference(this.shot_vector()).unit();
